...
int BTF::resolve_args(std::string &func, std::map<std::string, SizedType> &variables, bool ret)
{
  __s32 id, max = (__s32) btf__get_nr_types(btf);
  ...
  for (id = 1; id <= max; id++)
  {
    ...
    for (; j < vlen; j++, p++)
    {
      ...
      stype.tracing_arg = j;
      variables.insert({std::string("$") + str, stype});
    }
    ...
    if (ret)
    {
      ...
      stype.tracing_arg = j;
      variables.insert({std::string("$ret"), stype});
    }
